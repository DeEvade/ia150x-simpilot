...
import { findICAO, parseAction } from "./utils"
...
app.get("/flightdata", async (req: Request, res: Response) => {
    const flightDataStore = FlightDataStore.getInstance();
    const flightData = flightDataStore.getAllFlightData();

    const callsigns = Array.from(flightData.keys());
    const icaoMap: Record<string, string> = await findICAO(callsigns);

    const response = Array.from(flightData.entries()).map(([key, data]: [string, FlightData]) => ({
        callsign: key,
        callsignICAO: icaoMap[key] || null,
        lat: data.lat,
        lon: data.lon,
        alt: data.alt,
        hdg: data.hdg,
